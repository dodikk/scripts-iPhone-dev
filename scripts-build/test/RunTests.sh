...
/bin/bash "$SCRIPTS_ROOT_DIR/simulator/CleanTestReports.sh"
    ...
    /bin/bash "$PWD/RunRIApiTest.sh" $IOS_VERSION $CONFIGURATION
    if [ "$?" -ne "0" ]; then
       echo "[!!! ERROR !!!] : RunRIApiTest.sh failed"
       exit 1
    fi

#    /bin/bash "$PWD/RunOmnitureTest.sh" $IOS_VERSION $CONFIGURATION
#    if [ "$?" -ne "0" ]; then 
#       echo "[!!! ERROR !!!] : RunOmnitureTest.sh failed"
#       exit 1
#    fi
